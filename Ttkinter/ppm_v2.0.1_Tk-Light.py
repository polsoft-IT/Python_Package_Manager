...
class AboutDialog(tk.Toplevel):
    ...
    def init_ui(self):
        ...
        lbl_logo = tk.Label(self, text="📦", font=("Segoe UI", 38), bg="#ffffff", fg="#0f172a")
        lbl_logo.pack(pady=(20, 2))

        lbl_title = tk.Label(self, text=METADATA["name"], font=("Segoe UI", 14, "bold"), bg="#ffffff", fg="#0f172a")
        lbl_title.pack()

        lbl_version = tk.Label(self, text=f"v{METADATA['version']}", font=("Segoe UI", 10, "bold"), bg="#ffffff", fg="#64748b")
        lbl_version.pack(pady=(0, 5))

        lbl_desc = tk.Label(self, text=lang["about_msg"], font=("Segoe UI", 11), bg="#ffffff", fg="#475569", wraplength=420, justify="center")
        lbl_desc.pack(pady=(0, 15))

        meta_frame = tk.Frame(self, bg="#f8fafc", bd=1, relief="solid")
        ...
        for idx, (label_text, value_text) in enumerate(rows):
            lbl_key = tk.Label(meta_frame, text=f"{label_text}:", font=("Segoe UI", 10, "bold"), bg="#f8fafc", fg="#64748b", anchor="w")
            lbl_key.grid(row=idx, column=0, sticky="w", padx=(15, 10), pady=4)

            is_link = "http" in value_text or "@" in value_text
            val_fg = "#1d4ed8" if is_link else "#0f172a"
            lbl_val = tk.Label(meta_frame, text=value_text, font=("Segoe UI", 10), bg="#f8fafc", fg=val_fg, anchor="w")
...
class PackageRowTk(tk.Frame):
    def __init__(self, parent, package_name, is_installed, install_callback, current_lang, installed_version=""):
        super().__init__(parent, bg="#ffffff", bd=1, relief="solid", highlightthickness=0)
        ...
        self.lbl_name = tk.Label(self, text=package_name, font=("Segoe UI", 11, "bold"), bg="#ffffff", fg="#0f172a", anchor="w")
        self.lbl_name.pack(side="left", padx=15, fill="x", expand=True)

        self.lbl_status = tk.Label(self, font=("Segoe UI", 10, "bold"), bg="#ffffff", anchor="e")
    ...
    def update_ui(self):
        ...
        if self.is_installed:
            ...
            self.lbl_status.config(text=f"✓ {lang['installed']}{version_str}", fg="#16a34a")
...
class InstallerAppTk:
    ...
    def init_styles(self):
        ...
        self.style.configure(".", font=("Segoe UI", 10), background="#f8fafc", foreground="#1e293b")
        self.style.configure("TButton", font=("Segoe UI", 9, "bold"), background="#e2e8f0", foreground="#0f172a", borderwidth=0, padding=6)
        self.style.map("TButton", background=[("hover", "#cbd5e1"), ("disabled", "#f1f5f9")], foreground=[("disabled", "#94a3b8")])
        
        self.style.configure("Primary.TButton", background="#3498db", foreground="white")
        self.style.map("Primary.TButton", background=[("hover", "#2980b9")])

        self.style.configure("Header.TLabel", font=("Segoe UI", 16, "bold"), background="#f8fafc", foreground="#0f172a")
    ...
    def init_ui(self):
        ...
        # Top Bar
        top_frame = tk.Frame(self.root, bg="#f8fafc")
        top_frame.pack(fill="x", padx=20, pady=(15, 5))

        self.lbl_header = tk.Label(top_frame, text=LANGUAGES[self.current_lang]["header"], font=("Segoe UI", 16, "bold"), bg="#f8fafc", fg="#0f172a")
        ...
        self.search_ctrl = tk.Entry(search_frame, textvariable=self.search_var, font=("Segoe UI", 10), bg="#ffffff", fg="#0f172a", bd=1, relief="solid")
        ...
        self.search_ctrl.config(fg="#94a3b8")
        self.search_ctrl.bind("<FocusIn>", self.clear_placeholder)
        self.search_ctrl.bind("<FocusOut>", self.set_placeholder)

        self.lbl_interpreter = tk.Label(search_frame, text=LANGUAGES[self.current_lang]["python_exec_label"], font=("Segoe UI", 9, "bold"), bg="#f8fafc", fg="#475569")
        self.lbl_interpreter.pack(side="left", padx=(0, 5))

        self.txt_interpreter = tk.Entry(search_frame, font=("Segoe UI", 10), bg="#f1f5f9", fg="#475569", bd=1, relief="solid")
        ...
        # Main Splitter Area
        self.pane = tk.PanedWindow(self.root, orient="horizontal", bd=0, bg="#cbd5e1", sashwidth=3)
        self.pane.pack(fill="both", expand=True, padx=20, pady=10)

        # Left panel: Categories
        left_panel = tk.Frame(self.pane, bg="#f8fafc")
        self.lbl_cat = tk.Label(left_panel, text=LANGUAGES[self.current_lang]["categories"], font=("Segoe UI", 9, "bold"), bg="#f8fafc", fg="#475569")
        self.lbl_cat.pack(anchor="w", pady=(0, 2))
        
        self.category_list = tk.Listbox(left_panel, font=("Segoe UI", 10), bg="#ffffff", fg="#0f172a", bd=1, relief="solid", highlightthickness=0, selectbackground="#3498db", selectforeground="white")
        ...
        right_panel = tk.Frame(self.pane, bg="#f8fafc")
        self.lbl_right_header = tk.Label(right_panel, text=LANGUAGES[self.current_lang]["select_category"], font=("Segoe UI", 11, "bold"), bg="#f8fafc", fg="#0f172a")
        ...
        # Debug Console
        self.debug_console = tk.Text(self.root, height=8, font=("Consolas", 10), bg="#f1f5f9", fg="#0f172a", bd=1, relief="solid")
        
        # Status Bar
        status_frame = tk.Frame(self.root, bg="#f8fafc", bd=1, relief="sunken")
        status_frame.pack(fill="x", side="bottom")

        self.lbl_status = tk.Label(status_frame, text=LANGUAGES[self.current_lang]["status_ready"], bg="#f8fafc", fg="#0f172a", font=("Segoe UI", 9))
        self.lbl_status.pack(side="left", padx=5, pady=2)
    ...
    def clear_placeholder(self, event=None):
        if self.search_var.get() == self.placeholder_text:
            self.search_var.set("")
            self.search_ctrl.config(fg="#0f172a")
    ...
    def refresh_package_view(self):
        ...
        if not packages_to_show:
            lbl_empty = tk.Label(self.scroll_frame.scrollable_frame, text=lang["no_search_results"], font=("Segoe UI", 10, "italic"), bg="#f8fafc", fg="#64748b")
