...
LANGUAGES = {
    "EN": {
        "title": "Python Package Installer (Tkinter)",
    ...
    "PL": {
        "title": "Instalator Pakietów Python (Tkinter)",
...
# --- KONFIGURACJA KOLORÓW DLA TEMATÓW ---
THEMES = {
    "dark": {
        "bg_main": "#000000",
        "bg_card": "#1e1e1e",
...
class AboutDialog(tk.Toplevel):
    def __init__(self, parent, lang_code, theme_name):
    ...
    def init_ui(self):
        ...
        lbl_logo = tk.Label(self, text="📦", font=("Segoe UI", 38), bg=colors["bg_card"], fg=colors["fg_main"])
        lbl_logo.pack(pady=(20, 2))

        lbl_title = tk.Label(self, text=METADATA["name"], font=("Segoe UI", 14, "bold"), bg=colors["bg_card"], fg=colors["fg_main"])
        lbl_title.pack()

        lbl_version = tk.Label(self, text=f"v{METADATA['version']}", font=("Segoe UI", 10, "bold"), bg=colors["bg_card"], fg=colors["fg_muted"])
        lbl_version.pack(pady=(0, 5))

        lbl_desc = tk.Label(self, text=lang["about_msg"], font=("Segoe UI", 11), bg=colors["bg_card"], fg=colors["fg_main"], wraplength=420, justify="center")
        lbl_desc.pack(pady=(0, 15))

        meta_frame = tk.Frame(self, bg=colors["bg_main"], bd=1, relief="solid", highlightbackground=colors["border"])
        ...
        for idx, (label_text, value_text) in enumerate(rows):
            lbl_key = tk.Label(meta_frame, text=f"{label_text}:", font=("Segoe UI", 10, "bold"), bg=colors["bg_main"], fg=colors["fg_muted"], anchor="w")
            lbl_key.grid(row=idx, column=0, sticky="w", padx=(15, 10), pady=4)

            is_link = "http" in value_text or "@" in value_text
            val_fg = "#00adb5" if is_link else colors["fg_main"]
            lbl_val = tk.Label(meta_frame, text=value_text, font=("Segoe UI", 10), bg=colors["bg_main"], fg=val_fg, anchor="w")
...
class ScrollableFrame(tk.Frame):
    def __init__(self, container, theme_name, *args, **kwargs):
        super().__init__(container, *args, **kwargs)
        colors = THEMES[theme_name]
        self.canvas = tk.Canvas(self, borderwidth=0, highlightthickness=0, bg=colors["bg_main"])
    ...
    def update_theme(self, theme_name):
        colors = THEMES[theme_name]
        self.canvas.config(bg=colors["bg_main"])
        self.scrollable_frame.config(bg=colors["bg_main"])


class PackageRowTk(tk.Frame):
    def __init__(self, parent, package_name, is_installed, install_callback, current_lang, theme_name, installed_version=""):
        super().__init__(parent, bg=THEMES[theme_name]["bg_card"], bd=1, relief="solid", highlightthickness=0)
        ...
        self.lbl_name = tk.Label(self, text=package_name, font=("Segoe UI", 11, "bold"), bg=colors["bg_card"], fg=colors["fg_main"], anchor="w")
        self.lbl_name.pack(side="left", padx=15, fill="x", expand=True)

        self.lbl_status = tk.Label(self, font=("Segoe UI", 10, "bold"), bg=colors["bg_card"], anchor="e")
    ...
    def update_ui(self):
        ...
        if self.is_installed:
            ...
            self.lbl_status.config(text=f"✓ {lang['installed']}{version_str}", fg=colors["success"])
            self.lbl_status.pack(side="left", padx=10)
            self.config(highlightbackground=colors["success_border"], highlightcolor=colors["success_border"], highlightthickness=1)
...
class InstallerAppTk:
    def __init__(self, root):
        ...
        self.current_theme = "dark"  # Domyślny motyw (czarny)
    ...
    def apply_theme_styles(self):
        ...
        self.style.configure(".", font=("Segoe UI", 10), background=colors["bg_main"], foreground=colors["fg_main"])
        
        self.style.configure("TButton", font=("Segoe UI", 9, "bold"), background=colors["bg_card"], foreground=colors["fg_main"], borderwidth=1, bordercolor=colors["border"], padding=6)
        self.style.map("TButton", background=[("hover", colors["bg_hover"]), ("disabled", colors["bg_main"])], foreground=[("disabled", colors["fg_muted"])])
        
        self.style.configure("TScrollbar", background=colors["bg_card"], troughcolor=colors["bg_main"], borderwidth=0, arrowsize=12)
        self.style.map("TScrollbar", background=[("hover", colors["bg_hover"])])
        
        self.style.configure("TProgressbar", background=colors["fg_muted"], troughcolor=colors["bg_card"], borderwidth=0)
    ...
    def init_ui(self):
        ...
        # Top Bar
        self.top_frame = tk.Frame(self.root, bg=colors["bg_main"])
        self.top_frame.pack(fill="x", padx=20, pady=(15, 5))

        self.lbl_header = tk.Label(self.top_frame, text=LANGUAGES[self.current_lang]["header"], font=("Segoe UI", 16, "bold"), bg=colors["bg_main"], fg=colors["fg_main"])
        self.lbl_header.pack(side="left")

        self.btn_about = ttk.Button(self.top_frame, text="ℹ️", width=4, command=self.show_about_dialog)
        self.btn_about.pack(side="right", padx=2)

        self.btn_theme = ttk.Button(self.top_frame, text="🌙 / ☀️", command=self.toggle_theme)
        self.btn_theme.pack(side="right", padx=2)

        self.btn_lang = ttk.Button(self.top_frame, text="🌐 EN / PL", command=self.toggle_language)
        self.btn_lang.pack(side="right", padx=2)

        self.btn_toggle_debug = ttk.Button(self.top_frame, text=LANGUAGES[self.current_lang]["btn_debug_show"], command=self.toggle_debug_panel)
        self.btn_toggle_debug.pack(side="right", padx=2)

        self.btn_refresh = ttk.Button(self.top_frame, text=LANGUAGES[self.current_lang]["refresh_btn"], command=self.refresh_installed_packages)
        self.btn_refresh.pack(side="right", padx=2)

        # Search & Interpreter Bar
        self.search_frame = tk.Frame(self.root, bg=colors["bg_main"])
        self.search_frame.pack(fill="x", padx=20, pady=5)

        self.search_var = tk.StringVar()
        self.search_var.trace_add("write", self.on_search_text)
        
        self.search_ctrl = tk.Entry(self.search_frame, textvariable=self.search_var, font=("Segoe UI", 10), bg=colors["bg_card"], fg=colors["fg_main"], insertbackground=colors["fg_main"], bd=1, relief="solid")
        ...
        self.search_ctrl.config(fg=colors["fg_placeholder"])
        self.search_ctrl.bind("<FocusIn>", self.clear_placeholder)
        self.search_ctrl.bind("<FocusOut>", self.set_placeholder)

        self.lbl_interpreter = tk.Label(self.search_frame, text=LANGUAGES[self.current_lang]["python_exec_label"], font=("Segoe UI", 9, "bold"), bg=colors["bg_main"], fg=colors["fg_muted"])
        self.lbl_interpreter.pack(side="left", padx=(0, 5))

        self.txt_interpreter = tk.Entry(self.search_frame, font=("Segoe UI", 10), bg=colors["bg_main"], fg=colors["fg_muted"], bd=1, relief="solid")
        ...
        self.btn_choose_python = ttk.Button(self.search_frame, text=LANGUAGES[self.current_lang]["choose_interpreter_btn"], command=self.choose_python_executable)
        self.btn_choose_python.pack(side="left")

        # Main Splitter Area
        self.pane = tk.PanedWindow(self.root, orient="horizontal", bd=0, bg=colors["sash"], sashwidth=3)
        self.pane.pack(fill="both", expand=True, padx=20, pady=10)

        # Left panel: Categories
        self.left_panel = tk.Frame(self.pane, bg=colors["bg_main"])
        self.lbl_cat = tk.Label(self.left_panel, text=LANGUAGES[self.current_lang]["categories"], font=("Segoe UI", 9, "bold"), bg=colors["bg_main"], fg=colors["fg_muted"])
        self.lbl_cat.pack(anchor="w", pady=(0, 2))
        
        self.category_list = tk.Listbox(self.left_panel, font=("Segoe UI", 10), bg=colors["bg_card"], fg=colors["fg_main"], bd=1, relief="solid", highlightthickness=0, selectbackground=colors["fg_main"], selectforeground=colors["bg_main"])
        self.category_list.pack(fill="both", expand=True)
        self.category_list.bind("<<ListboxSelect>>", self.on_category_select)
        
        self.pane.add(self.left_panel, width=260)

        # Right panel: Packages View
        self.right_panel = tk.Frame(self.pane, bg=colors["bg_main"])
        self.lbl_right_header = tk.Label(self.right_panel, text=LANGUAGES[self.current_lang]["select_category"], font=("Segoe UI", 11, "bold"), bg=colors["bg_main"], fg=colors["fg_main"])
        self.lbl_right_header.pack(anchor="w", pady=(0, 4))

        self.scroll_frame = ScrollableFrame(self.right_panel, self.current_theme)
        self.scroll_frame.pack(fill="both", expand=True)
        
        self.pane.add(self.right_panel, width=660)

        # Debug Console
        self.debug_console = tk.Text(self.root, height=8, font=("Consolas", 10), bg=colors["bg_card"], fg=colors["fg_main"], insertbackground=colors["fg_main"], bd=1, relief="solid")
        
        # Status Bar
        self.status_frame = tk.Frame(self.root, bg=colors["bg_main"], bd=1, relief="flat")
        self.status_frame.pack(fill="x", side="bottom")

        self.lbl_status = tk.Label(self.status_frame, text=LANGUAGES[self.current_lang]["status_ready"], bg=colors["bg_main"], fg=colors["fg_muted"], font=("Segoe UI", 9))
        self.lbl_status.pack(side="left", padx=5, pady=4)

        self.progress_bar = ttk.Progressbar(self.status_frame, orient="horizontal", length=160, mode="determinate")
    ...
    def clear_placeholder(self, event=None):
        ...
        if self.search_var.get() == self.placeholder_text:
            self.search_var.set("")
            self.search_ctrl.config(fg=colors["fg_main"])
    ...
    def toggle_theme(self):
        ...
        # Odświeżenie podstawowych widgetów Tkinter
        self.root.config(bg=colors["bg_main"])
        self.top_frame.config(bg=colors["bg_main"])
        self.lbl_header.config(bg=colors["bg_main"], fg=colors["fg_main"])
        self.search_frame.config(bg=colors["bg_main"])
        self.search_ctrl.config(bg=colors["bg_card"], fg=colors["fg_main"], insertbackground=colors["fg_main"])
        self.lbl_interpreter.config(bg=colors["bg_main"], fg=colors["fg_muted"])
        self.txt_interpreter.config(bg=colors["bg_main"], fg=colors["fg_muted"])
        self.pane.config(bg=colors["sash"])
        self.left_panel.config(bg=colors["bg_main"])
        self.lbl_cat.config(bg=colors["bg_main"], fg=colors["fg_muted"])
        self.category_list.config(bg=colors["bg_card"], fg=colors["fg_main"], selectbackground=colors["fg_main"], selectforeground=colors["bg_main"])
        self.right_panel.config(bg=colors["bg_main"])
        self.lbl_right_header.config(bg=colors["bg_main"], fg=colors["fg_main"])
        self.debug_console.config(bg=colors["bg_card"], fg=colors["fg_main"], insertbackground=colors["fg_main"])
        self.status_frame.config(bg=colors["bg_main"])
        self.lbl_status.config(bg=colors["bg_main"], fg=colors["fg_muted"])

        # Jeśli wyszukiwarka wyświetla placeholder, ustaw jego poprawny kolor
        if self.search_var.get() == self.placeholder_text:
            self.search_ctrl.config(fg=colors["fg_placeholder"])

        self.scroll_frame.update_theme(self.current_theme)
    ...
    def show_about_dialog(self):
        AboutDialog(self.root, self.current_lang, self.current_theme)
    ...
    def refresh_package_view(self):
        ...
        if not packages_to_show:
            lbl_empty = tk.Label(self.scroll_frame.scrollable_frame, text=lang["no_search_results"], font=("Segoe UI", 10, "italic"), bg=colors["bg_main"], fg=colors["fg_placeholder"])
        ...
        for pkg in sorted(packages_to_show):
            ...
            PackageRowTk(self.scroll_frame.scrollable_frame, pkg, is_installed, self.handle_package_action, self.current_lang, self.current_theme, version)
