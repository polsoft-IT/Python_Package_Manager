...
LANGUAGES = {
    "EN": {
        "title": "Python Package Installer (Tkinter - Black Mode)",
    ...
    "PL": {
        "title": "Instalator Pakietów Python (Tkinter - Black Mode)",
...
class AboutDialog(tk.Toplevel):
    ...
    def init_ui(self):
        ...
        lbl_logo = tk.Label(self, text="📦", font=("Segoe UI", 38), bg="#121212", fg="#ffffff")
        lbl_logo.pack(pady=(20, 2))

        lbl_title = tk.Label(self, text=METADATA["name"], font=("Segoe UI", 14, "bold"), bg="#121212", fg="#ffffff")
        lbl_title.pack()

        lbl_version = tk.Label(self, text=f"v{METADATA['version']}", font=("Segoe UI", 10, "bold"), bg="#121212", fg="#888888")
        lbl_version.pack(pady=(0, 5))

        lbl_desc = tk.Label(self, text=lang["about_msg"], font=("Segoe UI", 11), bg="#121212", fg="#cccccc", wraplength=420, justify="center")
        lbl_desc.pack(pady=(0, 15))

        meta_frame = tk.Frame(self, bg="#000000", bd=1, relief="solid")
        ...
        for idx, (label_text, value_text) in enumerate(rows):
            lbl_key = tk.Label(meta_frame, text=f"{label_text}:", font=("Segoe UI", 10, "bold"), bg="#000000", fg="#888888", anchor="w")
            lbl_key.grid(row=idx, column=0, sticky="w", padx=(15, 10), pady=4)

            is_link = "http" in value_text or "@" in value_text
            val_fg = "#00adb5" if is_link else "#eeeeee"
            lbl_val = tk.Label(meta_frame, text=value_text, font=("Segoe UI", 10), bg="#000000", fg=val_fg, anchor="w")
...
class ScrollableFrame(tk.Frame):
    def __init__(self, container, *args, **kwargs):
        ...
        self.canvas = tk.Canvas(self, borderwidth=0, highlightthickness=0, bg="#000000")
...
class PackageRowTk(tk.Frame):
    def __init__(self, parent, package_name, is_installed, install_callback, current_lang, installed_version=""):
        super().__init__(parent, bg="#1e1e1e", bd=1, relief="solid", highlightthickness=0)
        ...
        self.lbl_name = tk.Label(self, text=package_name, font=("Segoe UI", 11, "bold"), bg="#1e1e1e", fg="#ffffff", anchor="w")
        self.lbl_name.pack(side="left", padx=15, fill="x", expand=True)

        self.lbl_status = tk.Label(self, font=("Segoe UI", 10, "bold"), bg="#1e1e1e", anchor="e")
    ...
    def update_ui(self):
        ...
        if self.is_installed:
            ...
            self.lbl_status.config(text=f"✓ {lang['installed']}{version_str}", fg="#00ff66")
            self.lbl_status.pack(side="left", padx=10)
            self.config(highlightbackground="#00aa44", highlightcolor="#00aa44", highlightthickness=1)
...
class InstallerAppTk:
    ...
    def init_styles(self):
        ...
        # Konfiguracja czystych, czarno-szarych stylów dla elementów Ttk
        self.style.configure(".", font=("Segoe UI", 10), background="#000000", foreground="#ffffff")
        
        self.style.configure("TButton", font=("Segoe UI", 9, "bold"), background="#1e1e1e", foreground="#ffffff", borderwidth=1, bordercolor="#333333", padding=6)
        self.style.map("TButton", background=[("hover", "#2d2d2d"), ("disabled", "#121212")], foreground=[("disabled", "#555555")])
        
        self.style.configure("TScrollbar", background="#1e1e1e", troughcolor="#000000", borderwidth=0, arrowsize=12)
        self.style.map("TScrollbar", background=[("hover", "#2d2d2d")])
        
        self.style.configure("TProgressbar", background="#555555", troughcolor="#1e1e1e", borderwidth=0)
    ...
    def init_ui(self):
        ...
        self.lbl_header = tk.Label(top_frame, text=LANGUAGES[self.current_lang]["header"], font=("Segoe UI", 16, "bold"), bg="#000000", fg="#ffffff")
        ...
        self.search_ctrl = tk.Entry(search_frame, textvariable=self.search_var, font=("Segoe UI", 10), bg="#1e1e1e", fg="#ffffff", insertbackground="white", bd=1, relief="solid")
        ...
        self.search_ctrl.config(fg="#777777")
        self.search_ctrl.bind("<FocusIn>", self.clear_placeholder)
        self.search_ctrl.bind("<FocusOut>", self.set_placeholder)

        self.lbl_interpreter = tk.Label(search_frame, text=LANGUAGES[self.current_lang]["python_exec_label"], font=("Segoe UI", 9, "bold"), bg="#000000", fg="#888888")
        self.lbl_interpreter.pack(side="left", padx=(0, 5))

        self.txt_interpreter = tk.Entry(search_frame, font=("Segoe UI", 10), bg="#000000", fg="#888888", bd=1, relief="solid")
        ...
        # Main Splitter Area
        self.pane = tk.PanedWindow(self.root, orient="horizontal", bd=0, bg="#2d2d2d", sashwidth=3)
        self.pane.pack(fill="both", expand=True, padx=20, pady=10)

        # Left panel: Categories
        left_panel = tk.Frame(self.pane, bg="#000000")
        self.lbl_cat = tk.Label(left_panel, text=LANGUAGES[self.current_lang]["categories"], font=("Segoe UI", 9, "bold"), bg="#000000", fg="#888888")
        self.lbl_cat.pack(anchor="w", pady=(0, 2))
        
        self.category_list = tk.Listbox(left_panel, font=("Segoe UI", 10), bg="#1e1e1e", fg="#ffffff", bd=1, relief="solid", highlightthickness=0, selectbackground="#ffffff", selectforeground="#000000")
        ...
        right_panel = tk.Frame(self.pane, bg="#000000")
        self.lbl_right_header = tk.Label(right_panel, text=LANGUAGES[self.current_lang]["select_category"], font=("Segoe UI", 11, "bold"), bg="#000000", fg="#ffffff")
        ...
        # Debug Console
        self.debug_console = tk.Text(self.root, height=8, font=("Consolas", 10), bg="#1e1e1e", fg="#eeeeee", insertbackground="white", bd=1, relief="solid")
        
        # Status Bar
        status_frame = tk.Frame(self.root, bg="#000000", bd=1, relief="flat")
        status_frame.pack(fill="x", side="bottom")

        self.lbl_status = tk.Label(status_frame, text=LANGUAGES[self.current_lang]["status_ready"], bg="#000000", fg="#888888", font=("Segoe UI", 9))
    ...
    def clear_placeholder(self, event=None):
        if self.search_var.get() == self.placeholder_text:
            self.search_var.set("")
            self.search_ctrl.config(fg="#ffffff")
    ...
    def refresh_package_view(self):
        ...
        if not packages_to_show:
            lbl_empty = tk.Label(self.scroll_frame.scrollable_frame, text=lang["no_search_results"], font=("Segoe UI", 10, "italic"), bg="#000000", fg="#555555")
